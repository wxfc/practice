...
function startupGenerator(arr, arr2) {
    let thisRandomElement = arr[Math.floor(Math.random() * arr.length)]
    let thatRandomElement = arr2[Math.floor(Math.random() * arr2.length)]
    console.log(`${thisRandomElement} for ${thatRandomElement}`)

}

startupGenerator(thisArray, thatArray)